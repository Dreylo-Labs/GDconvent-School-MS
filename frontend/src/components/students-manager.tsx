...
export function StudentsManager() {
  ...
  return <>
    ...
    <section className="panel resource studentsResource">
      ...
      <div className="tableWrap"><table><thead>{user?.role === 'ACCOUNTANT' ? <tr><th>Admission no.</th><th>Student</th><th>Class</th><th>Parent</th><th>Billed</th><th>Collected</th><th>Outstanding</th><th>Advance</th><th>Action</th></tr> : <tr><th>Admission no.</th><th>Student</th><th>Class</th><th>Father</th><th>Joined</th><th>Action</th></tr>}</thead><tbody>{loading ? <tr><td colSpan={user?.role === 'ACCOUNTANT' ? 9 : 6}><div className="tableState">Loading students…</div></td></tr> : students.length ? students.map(student => user?.role === 'ACCOUNTANT' ? <tr key={student.id}><td className="mono">{student.admissionNo}</td><td><strong>{student.firstName} {student.lastName}</strong></td><td>{student.class ? classLabel(student.class) : <span className="mutedText">Unassigned</span>}</td><td>{student.fatherName || student.guardianName}<small className="cellSubtext">{student.fatherPhone || student.guardianPhone}</small></td><td>₹{(student.billed ?? 0).toLocaleString('en-IN')}</td><td className="greenText">₹{(student.collected ?? 0).toLocaleString('en-IN')}</td><td><strong className={(student.outstanding ?? 0) > 0 ? 'redText' : ''}>₹{(student.outstanding ?? 0).toLocaleString('en-IN')}</strong></td><td className="blueText">₹{(student.advanceBalance ?? 0).toLocaleString('en-IN')}</td><td><Link className="viewButton" href={`/fees/students/${student.id}`}><EyeIcon />Fee history</Link></td></tr> : <tr key={student.id}><td className="mono">{student.admissionNo}</td><td><strong>{student.firstName} {student.lastName}</strong><small className="cellSubtext">{student.phone || 'No student phone added'}</small></td><td>{student.class ? classLabel(student.class) : <span className="mutedText">Unassigned</span>}</td><td>{student.fatherName || student.guardianName}<small className="cellSubtext">{student.fatherPhone || student.guardianPhone}</small></td><td>{new Intl.DateTimeFormat('en-IN',{dateStyle:'medium'}).format(new Date(student.createdAt))}</td><td><Link className="viewButton" href={`/students/${student.id}`}><EyeIcon />View</Link></td></tr>) : <tr><td colSpan={user?.role === 'ACCOUNTANT' ? 9 : 6}><div className="tableState">No students match these filters.</div></td></tr>}</tbody></table></div>
...
function StudentDialog({ classes, onClose, onCreated }: { classes: SchoolClass[]; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  async function createStudent(event: FormEvent<HTMLFormElement>) {
    ...
    body.feeExempt = form.get('feeExempt') === 'on';
    for (const key of ['phone','address','classId','feeExemptReason','feeExemptFrom','feeExemptTo']) if (!body[key]) delete body[key];
  ...
  return <div className="dialogBackdrop" onMouseDown={onClose}><div className="facilityDialog studentDialog" role="dialog" aria-modal="true" onMouseDown={event => event.stopPropagation()}><div className="dialogTitle"><div><h2>Add student</h2><p>Generate the next number for any admission year, or enter a custom value.</p></div><button onClick={onClose} aria-label="Close"><XMarkIcon /></button></div><form className="facilityForm" onSubmit={createStudent}>
    ...
    <label>Student phone<input name="phone" inputMode="tel" /></label>
    <label>Address<textarea name="address" rows={2} /></label>
    <div className="formColumns"><label>Father&apos;s name<input name="fatherName" required /></label><label>Father&apos;s number<input name="fatherPhone" inputMode="tel" required /></label></div>
    <div className="formColumns"><label>Mother&apos;s name<input name="motherName" required /></label><label>Mother&apos;s number<input name="motherPhone" inputMode="tel" required /></label></div>
    <fieldset className="feeStructurePicker"><legend>Fee support (optional)</legend><label><input type="checkbox" name="feeExempt"/><span><strong>Full fee exemption</strong><small>Use for faculty children or another approved free-fee case.</small></span></label><div className="formColumns"><label>Reason<input name="feeExemptReason" placeholder="Faculty child, management approval…"/></label><label>Effective from<input name="feeExemptFrom" type="date"/></label></div><label>Effective until<input name="feeExemptTo" type="date"/></label></fieldset>
